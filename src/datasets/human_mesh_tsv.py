    ...
    def __getitem__(self, idx):
        ...
        if self.is_train:
            ...
            for i in range(num_joints):
                if np.random.random() < mvm_percent:
                    mjm_mask[i, :] = 0.0
            #
        ...
        if self.is_train:
            ...
            # pb = np.random.random_sample()
            # masked_num = int(pb * mvm_percent * num_feature)  # at most x% of the vertices could be masked
            # indices = np.random.choice(np.arange(num_feature), replace=False, size=masked_num)
            # mlfm_mask[indices, :] = 0.0
        mlfm_mask = torch.from_numpy(mlfm_mask).float()

        mmfm_mask = np.ones((196, 1))
        if self.is_train:
            num_feature = 196
            for i in range(num_feature):
                if np.random.random() < mvm_percent:
            ...
            # masked_num = int(pb * mvm_percent * num_feature)  # at most x% of the vertices could be masked
            # indices = np.random.choice(np.arange(num_feature), replace=False, size=masked_num)
            # mmfm_mask[indices, :] = 0.0
        mmfm_mask = torch.from_numpy(mmfm_mask).float()

        mhfm_mask = np.ones((196, 1))
        if self.is_train:
            num_feature = 196
            # pb = np.random.random_sample()
            # masked_num = int(pb * mvm_percent * num_feature)  # at most x% of the vertices could be masked
            ...
            for i in range(num_feature):
                if np.random.random() < mvm_percent:
                    mhfm_mask[i, :] = 0.0
            # mhfm_mask[indices, :] = 0.0
        mhfm_mask = torch.from_numpy(mhfm_mask).float()
        ...
        meta_data['mlfm_mask'] = mlfm_mask
        meta_data['mmfm_mask'] = mmfm_mask
        meta_data['mhfm_mask'] = mhfm_mask
