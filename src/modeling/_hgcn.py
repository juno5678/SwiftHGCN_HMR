...
from src.modeling._gcnn import GraphLinear, GraphConvolution
from src.modeling.__hgcn_gcn_conv import HGConv_edge_to_node, HGConv_node_to_edge
from torch_geometric.nn import GCNConv
from src.modeling.__hgcn_gcn import GCN_block
from src.modeling.__hgcn_gcn_conv import ModulatedHyperGraphConv
...
class HGGCN(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, hidden_channels=64, num_layers=2):
        ...
        self.gcn = GCN_block(self.adjacency_matrix, input_dim=hidden_channels, hidden_dim=384, output_dim=hidden_channels, drop_path=0.15)
    ...
    def forward(self, x, hyperedge_index, graph_index, hyperedge_attr=None):
        ...
        output = self.gelu(self.norm(output.view(B, num_edges, -1)))
...
class HGGCNBlock(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, hidden_channels=64, num_layers=2):
        ...
        incident_mx = torch.load('overlap_incident_mx.pt')
        self.conv1 = ModulatedHyperGraphConv(in_channels=hidden_channels, out_channels=hidden_channels, H=incident_mx, hidden_channels=hidden_channels)
        # self.conv2 = HGGCN(hidden_channels, hidden_channels, hidden_channels)

        mesh_type = "body"
        self.conv2 = ModulatedHyperGraphConv(in_channels=hidden_channels, out_channels=hidden_channels, H=incident_mx, hidden_channels=hidden_channels)
    ...
    def forward(self, x, hyperedge_index, graph_index, hyperedge_attr=None):
        ...
        y = self.conv1(y)
        # y = self.gelu(self.norm2(y))
        y = self.conv2(y)
        trans_y = self.gelu(self.norm3(y)).transpose(1,2)
...
class HyperGraphResBlock(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, hidden_channels=64, num_layers=2):
        ...
        self.conv = ModulatedHyperGraphConv(in_channels=hidden_channels//2, out_channels=hidden_channels, hidden_channels=hidden_channels//4)
        # self.conv = HGCN(out_channels // 2, out_channels // 2, mesh_type)
        self.lin2 = GraphLinear(hidden_channels, out_channels)
        self.skip_conv = GraphLinear(in_channels, out_channels)
        # print('Use BertLayerNorm in GraphResBlock')
