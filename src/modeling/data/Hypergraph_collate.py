...
from torch_geometric.data.collate import repeat_interleave, _batch_and_ptr, get_incs
...
def hg_collate(
    ...
    for out_store in out.stores:  # type: ignore
        ...
        for attr in stores[0].keys():
            ...
            value, slices, incs = _hg_collate(attr, values, data_list, stores,
                                           increment)
...
def _hg_collate(
    ...
    if isinstance(elem, Tensor) and not is_sparse(elem):
        ...
        slices = cumsum(sizes)
        if increment:
            incs = get_hg_incs(key, values, data_list, stores)
            if incs.dim() > 1 or int(incs[-1]) != 0:
                # values = [
                #     value + inc.to(value.device)
                #     for value, inc in zip(values, incs)
                # ]
                values = [torch.stack((value[0] + hg_inc[0], value[1] + hg_inc[1]))
                for value, hg_inc in zip(values, incs)]
        ...
        for key in elem.keys():
            value_dict[key], slice_dict[key], inc_dict[key] = _hg_collate(
        ...
        for i in range(len(elem)):
            value, slices, incs = _hg_collate(key, [v[i] for v in values],
...
def get_hg_incs(key, values: List[Any], data_list: List[BaseData],
    ...
    if key == 'edge_index':
        repeats_node = torch.tensor([int(value[0].max())+1 for value in values])
        repeats_edge = torch.tensor([int(value[1].max()) + 1 for value in values])
        return torch.stack([cumsum(repeats_node[:-1]), cumsum(repeats_edge[:-1])], dim=1)
    else:
